...
export class RegisterComponent {
  ...
  validateStep1() {
    ...
    // If all validations pass, proceed to OTP step
    this.nextStep();
  }
  ...
  validateOTP() {
    ...
    if (enteredOTP.length !== 6) {
      this.toastr.error('Please enter the complete 6-digit OTP', 'Error');
      return;
    }
    ...
    if (enteredOTP == this.generatedOTP) {
      ...
      this.toastr.error('Invalid OTP. Please try again.', 'Error', { progressBar: true, tapToDismiss: true });
    }
  }
  ...
  resendOTP(event: Event) {
    ...
    this.generateOTP();
    this.toastr.success('New OTP has been sent to your email', 'Success', { progressBar: true, tapToDismiss: true });
  }
